...
def require_care_or_admin(current_user: User = Depends(get_current_user)) -> User:
    """
...
def get_care_providers(
    ...
    query = db.query(User).filter(User.role == UserRole.CARE_PROVIDER, User.is_active == True)

    if specialty:
        from app.db.models import SpecialistType, CareProviderProfile
        try:
            specialty_enum = SpecialistType(specialty.upper())
            query = query.join(CareProviderProfile).filter(CareProviderProfile.specialty == specialty_enum)
        except ValueError:
            raise HTTPException(
